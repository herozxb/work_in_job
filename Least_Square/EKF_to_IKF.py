...
def non_linear_least_squares(f, params, data, max_iter=1000, tol=1e-3):
  ...
  for i in range(1000):
    ...
    #K_new = np.linalg.inv( H.T @ np.linalg.inv(R) @ H + np.linalg.inv(P_hat) ) * H.T  @ np.linalg.inv(R) 

    delta_params_by_K = K @ r

    #delta_params_by_K_new = K_new @ r


    print("============delta_params_by_K[1]=============")
    print(delta_params_by_K)
    #print(delta_params_by_K_new)
    ...
    #H = ekf_measurement_linearization_10x2(x_hat[0][0], x_hat[1][0], data[0], data[1])
    #x_hat, P_hat = ekf_update(x_hat, P_hat, z,  H)
